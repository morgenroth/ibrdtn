...
import de.tubs.ibr.dtn.service.NativeLibraryProcess.ProcessListener;
...
public class DaemonManager implements ProcessListener {
	...
	private NativeLibraryProcess _process = null;
	...
	private EventListener _listener = new EventListener() {
		public void eventRaised(Event evt)
		{
			...
			if (Log.isLoggable(TAG, Log.DEBUG))
				Log.d(TAG, "EVENT intent broadcasted: " + evt.getName() + "; Action: " + evt.getAction());
		}
	};

	/**
	 * Resets a eventually running daemon without a connection to any process.
	 */
	public synchronized void reset(Context context)
	{
		...
		_process = new NativeLibraryProcess(context, this);
		_process.kill();
	}
	...
	public void onProcessStop()
	{
		...
		if (getState().equals(DaemonState.ERROR))
			return;
